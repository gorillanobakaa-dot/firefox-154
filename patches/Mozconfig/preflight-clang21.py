...
===================================================================
Runs before ./mach build to catch & auto-fix known breakage patterns
for this specific "Gorilla Unleashed" Firefox 154 out-of-tree build.

WHY v2 EXISTS
-------------
v1 of this script (and a ~16-hour agent session run against this tree)
correctly *identified* most of the problems below, but the session still
failed because of process bugs, not knowledge gaps:
  - a fix was reverted back to the ORIGINAL broken text and nobody noticed
  - the same one-line cast was re-applied 3 separate times (stale reads)
  - "FIXED" was logged in a changelog file 4x with no corresponding edit
  - a no-op edit (oldString == newString) burned a full build cycle, twice
  - a "comprehensive summary" of fixes was persisted to long-term memory
    describing work that had not actually happened in the source tree
  - mozwebidlcodegen was invoked with the wrong PYTHONPATH twice before
    succeeding, on an error that was fully diagnosable up front

v2 adds a persistent JOURNAL (.preflight_state.json) that fingerprints
every file this script touches. On each run it can tell the difference
between "still broken", "fixed and still fixed", and "was fixed, then
SOMEONE REVERTED IT" — and it screams loudly about the third case,
because that's exactly what burned the most time last session.

IMPORTANT — SCOPE HONESTY:
The checks below (MaybeStorage, UnionMember::Construct, IPDL resolver
typedefs, FFmpeg %p/%d logging, StaticPrefList.yaml entries, the Wayland
GPU patch) are FIREFOX/MOZILLA-INTERNAL breakage patterns, most of them
specific to this hand-patched "Gorilla" tree. They are NOT documented on
releases.llvm.org under these names — they are downstream symptoms of
Clang 21's stricter generic rules. Section F below covers the actual
*generic* Clang 21.1.0 language-conformance changes from the real
release notes (releases.llvm.org/21.1.0), separately, so the two are not
conflated.

Install: drop in firefox-source/ and add to mach pre-build hook.
"""
import hashlib
import json
import re
...
def record_check(check_id: str, file_path: Path, status: str, detail: str = ""):
    ...
    is exactly the AddCertExceptionResolver bug from last session (fixed,
...
def append_changelog(kind: str, msg: str):
    """
    Dedup guard for the changelog file itself. Last session logged the exact
...
def safe_replace(path: Path, old: str, new: str, description: str) -> bool:
    ...
    editing agent working on this tree). Refuses:
      - no-op edits where old == new (burned 2 build cycles last session)
...
def check_ipc_signatures():
    ...
    # --- 9a. AddCertExceptionResolver: THE bug from last session -----------
    # Last session tried 6+ different wordings of this line across ~90 tool
    ...
    # Last session added a bare `this` argument to CreateDisconnected() calls
            ...
            if func_sigs:
                ...
                if "::" not in last_sig:
                    log_error(f"ContentParent.cpp:{line_no}: CreateDisconnected(..., this) is called "
                              ...
                              f"valid there. This is the exact bug from the last session that was "
        ...
        if m:
            ...
            if uses_notnull_uri:
                log_error("ContentChild.h: RecvLoadURIExternal takes NotNull<nsIURI*> for the uri "
                          "param. Last session's own generated-header check showed the real call "
...
def check_ffmpeg_logging_format():
    ...
    # journal per-file rather than per-line: last session re-fixed the exact
...
# surfaced only AFTER a successful IPDL/webidl regeneration last session
...
# Last session burned 2 of 3 attempts on a basic PYTHONPATH mistake: adding
...
def detect_build_loop(log_paths, min_repeats=2):
    ...
    AddCertExceptionResolver and the ContentChild scope issue last session,
...
def main():
    ...
    # paths this tree's agent sessions have used, check them for loops.
