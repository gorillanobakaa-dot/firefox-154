...
> **Role note / SUPERSEDED VERDICT**: this audit's **PASS (2026-07-10) is
> retracted with hindsight** — it scanned the FTL set while the sed-rebrand
> damage (508 doubled strings, poisoned variant keys, 207 missing messages +
> 1 missing term) was PRESENT and undetected; the rules of that era did not
> check for those classes. The damage was found and repaired 2026-07-31
> (THEME_FIX_LOG §10, §22–23). Current rule-level state: `dual-track precheck
> FIrefox.154.Look` (2026-07-31) → 0 P0 / 0 P1, 6 benign upstream-TODO P2s.
> A fresh full audit is DUE after the next full build. Kept for audit-trail
> honesty: what a PASS missed is itself a lesson (see DB atom
> Scrubbed_Identifiers_In_Memory_Tiers — "plausible + specific ≠ true").
