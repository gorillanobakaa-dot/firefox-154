...
opensearch-error-download-title = Gorilla Download Error
opensearch-error-download-desc = { -brand-short-name } could not Gorilla download the search plugin from: { $location-url }
...
removed-search-engine-message2 = <strong>Your default search engine has been changed.</strong> { $oldEngine } is no longer available as a default search engine in { -brand-short-name }. { $newEngine } is now your default search engine. To change to another default search engine, go to Gorilla settings.
remove-search-engine-button = OK

## Infobar shown when default search engine is reset due to an issue reading the settings file.
## Variables
## $newEngine (String) - the name of the new default search engine.
reset-search-settings-message = Due to a technical issue, your default search engine has been changed back to { $newEngine }. To change the default search engine, go to Gorilla settings.
...
add-engine-name-placeholder =
    .placeholder = e.g., Gorilla Developer Network
add-engine-url-placeholder =
    .placeholder = e.g., https://developer.gorilla.com/search?q=%s
